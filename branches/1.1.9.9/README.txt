version 1.1.9
. Refactoring the Sorter and Comparator code.
. Add a new property in row action: disable.

version 1.1.8
. Move the "dataRecords" "dataColumn" "dataColumnDecorate" attributes from
SmartTableGrid into SmartListTableTag
. Support to define the sortBy and sortOrder in PreferenceConfigure.
. Enable to define multi-table in one page.

version 1.1.7
. Enable to display the elements which be defined in the <pf:list> tag.
. Define a new resource bundle - SmartResourceBundle.

version 1.1.6
. Support i18n

version 1.1.5
. Define a new column decorate: SelectColumnDecorate
. Refactoring the code to support the row action in static/dynamic way. The row action be recognized as the column.
. Add a new method in ListTableAction: registerDataColumnDecorate.
. Define a new interface: IDataColumnDecorate which is a shadow of IEnableDecorate
...
2. Add it in your 鈥淲EB-INF/lib鈥�folder of web application
	...
	b)	Use the tag to display the list data in JSP
			...
			<pf:list action="/request/toList.action">  
			    <pf:table list="requests" cssClass="tableBasic">
			        ...
			        <pf:action>
			        	//add the options
			           <pf:option label=鈥滱ction鈥�name=鈥漚ction鈥�>
			           <pf:option label=鈥�---鈥�name=鈥�---鈥�>
			           <pf:option label=鈥漇top鈥�name=鈥漵top鈥�>
			        </pf:action>			        
			    </pf:table>
			</pf:list>
...
3 FAQ
	a)	How to define the row action?
		[A] use 鈥減f:action鈥�into the 鈥減f:table鈥�elements
			<pf:list action="/request/toList.action">
			    <pf:table list="requests" cssClass="tableBasic">
			        <pf:column name="id" value="ID"/>
