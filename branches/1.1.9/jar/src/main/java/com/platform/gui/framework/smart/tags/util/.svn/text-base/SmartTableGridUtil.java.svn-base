...
public class SmartTableGridUtil {
	 ...
	 * @throws Exception
	 */
	public static SmartTableGrid getSmartTable(PageContext pageContext , SmartTableContext context , IDataRecordList<?> dataRecords , IDataColumn dataColumn , IDataColumnDecorate columnDecorate) throws Exception{
		SmartTableGrid table = null;
		if(dataColumn != null){
			// if can not get the cells from the JSP, get it from the bean
			List<SmartCell> cells = getSmartCellList(pageContext , dataColumn , columnDecorate);
			
			// get the row data from the context
			List<SmartRow> rows = getSmartRowList(pageContext , context , dataRecords);
			
			// get data size
			Integer dataSize = getSmartDataSize(pageContext , dataRecords);
			
			// list table object
			table = new SmartTableGrid(context , rows , cells , dataSize);
		}
		return table;
	}
	 ...
	 * @throws Exception
	 */
	public static List<SmartCell> getSmartCellList(PageContext pageContext , IDataColumn dataColumn , IDataColumnDecorate columnDecorate) throws Exception{
        ...
        for(DataColumn column:columns){
            ...
            if(decorate == null){
            	...
            	if(decorate == null){
            		decorate = SmartColumnDecorateUtil.getDecorateFromBean(pageContext , columnDecorate , dataColumn , cell);
            	}
	...
	public static List<SmartRow> getSmartRowList(PageContext pageContext , SmartTableContext context , IDataRecordList<?> dataRecords) throws Exception{
		...
		FilterData fd = FilterDataUtil.getFilterData(pageContext);
	 ...
	 * @throws Exception
	 */
	public static Integer getSmartDataSize(PageContext pageContext , IDataRecordList<?> dataRecords) throws Exception{
		Integer dataSize = 0;
