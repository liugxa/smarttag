...
<table class="tableBasic" id="st_table" name="st_table">
			...
			<th class="${getThClass(context , cell)}" style="${getColumnStyle(cell)}" title="${getText(context , cell.title)}">
				...
				<#else>
					<#if cell.decorate?? && cell.decorate == "checkbox-all">
						<input type="checkbox" id="st_checkbox_all" name="st_checkbox_all_${cell.name}" onClick="st_checkbox_all_onClick(this);st_checkbox_all_onClick_after(this);"/>${getText(context , cell.label)}
					<#else>
	...
	<#list tableGrid.rows as row>
		<tr class="${getTrClass(row_index)}">
		    <#list tableGrid.cells as cell>
	    		<td class="${getTdClass(context , row_index , cell)}" style="${getColumnStyle(cell)}">${row.getColumnValue(row_index , cell)!'-'}</td>
		    </#list>
		</tr>
	</#list>
</table>