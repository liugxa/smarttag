...
public class RadioColumnDecorate extends DefaultColumnDecorate{
	...
	public String decorate(TableContext context , Integer index , Object bean , DataColumn column) throws Exception {
		...
		radio.setName(this.eval(columnName , index));
		radio.setID("st_radio_" + index);
		radio.setValue(columnValue.toString());

		radio.setChecked(this.isChecked(columnValue.toString()));
		radio.setOnClick("st_radio_onClick('" + formName + "' , this , " + JSONSerializer.toJSON(bean) + ");st_radio_onClick_after('" + formName + "' , this," + JSONSerializer.toJSON(bean) + ");");
