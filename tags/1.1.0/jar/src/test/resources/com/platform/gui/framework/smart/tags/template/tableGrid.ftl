 	...
 	<#list tableGrid.rows as row>
		...
		<tr class="${trColor}">
	  	...
	  	<#if tableGrid.multiple??>
	  		...
	  		<td>
	  			<#if row.isChecked>
	  				<input type="${tableGrid.multiple}" onChange="tableGridMultiple(this , '${url}' , '${primaryValue}')" checked>
	  			<#else>
	  				<input type="${tableGrid.multiple}" onChange="tableGridMultiple(this , '${url}' , '${primaryValue}')">
	  			</#if>
	  		</td>
	  	</#if>
