...
public class SmartColumnTag extends TagSupport{ 
		...
		try{
			...
			if(beans != null && beans.size() > 0){
				...
				if(obj instanceof IEnableDecorate){
					...
					decorate = this.getDecorateFromTag(obj , primaryKey , cell);
				}
			}
		}catch(Exception e){
			//forgot it!
	 ...
	 * @throws Exception
	 */
	private IColumnDecorate getDecorateFromTag(Object bean , String primaryKey , SmartCell cell) throws Exception{
		...
		//get the decorate by the property type
		if(cellFormat != null){
			Object property = PropertyUtils.getProperty(bean , cell.getName());
		...
		//get the url decorate
		if(cellUrl != null) decorate = new URLColumnDecorate(cellUrl);
