<script language="JavaScript">
	...
	//st_initialize('${context.name}');
	function st_initialize(_formId){
		var form = document.getElementById("st_form_" + _formId);
		var prefix = "st_checkbox_" + _formId;
		
		var checkboxAll = document.getElementById("st_checkbox_all_" + _formId);
		if(checkboxAll != null){
			//does the other checkbox has been checked?
			var isAllOtherChecked = st_checkbox_isAllChecked(_formId);
			if(isAllOtherChecked) checkboxAll.checked = true;
		}
	}
	
	function st_checkbox_isAllChecked(_formId){
		var r = false;
		
		//the form element and the checkbox's prefix name
		var form = document.getElementById("st_form_" + _formId);
		var prefix = "st_checkbox_" + _formId;
		...
		if(checkboxs != null){
			var checkboxAllId = "st_checkbox_all_" + _formId;
			for(var i = 0; i < checkboxs.length; i++) {
				var checkbox = checkboxs[i];
				//if the checkbox is not the checkbox-all item
				if(checkbox.id != checkboxAllId){
					if(checkbox.id.indexOf(prefix) != -1){
	...
	function st_checkbox_all_onClick(_formId , _this , _cellName){
		...
		//find all of the inputs which id's prefix is "st_checkbox_" + _formId.
		var checkboxs = form.getElementsByTagName("input");
		if(checkboxs != null){
			for(var i=0;i<checkboxs.length;i++){
				var checkbox = checkboxs[i];
	...
	function st_pagination_onClick(_formId , _this , _url){
		window.location.href = _url;
	}
	
	function st_tableHeader_onClick(_form , _this , _url){
		window.location.href = _url;
	}
	...
	function _st_filter_enable(_formId , _enable){
		...
		//if the filter div is exists, disable/enable the input/select elements
		if(filterDiv != null){
			//disable the inputs
