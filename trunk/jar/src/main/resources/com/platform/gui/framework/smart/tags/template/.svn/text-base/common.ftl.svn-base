	...
	<#if _tableGrid??>
		<#if sortOrder == "asc">
			...
			<#if sortBy == _cell.name>
				<#local parameters = "st_name=" + _context.name + "&st_sortBy=" + _cell.name + "&st_sortOrder=desc&st_page=0">
			<#else>
				<#local parameters = "st_name=" + _context.name + "&st_sortBy=" + _cell.name + "&st_sortOrder=asc&st_page=0">
			</#if>
		<#else>
			<#local parameters = "st_name=" + _context.name + "&st_sortBy=" + _cell.name + "&st_sortOrder=asc&st_page=0">
		</#if>
...
<#function getSortByValue _context _tableGrid>
	<#local sortBy = getSortBy(_context)>
	<#local sortByValue = getText(_context , "st.list.table.filter.any")>
	<#if sortBy?? && _tableGrid.cells??>
		<#list _tableGrid.cells as cell>
			<#if cell.name == sortBy>
				<#local sortByValue = getText(_context , cell.label)>
				<#break>
			</#if>
		</#list>
	</#if>
...
<#function getItemValue _context _item>
	<#local itemValue = getText(_context , "st.list.table.filter.any")>
	<#if _item.value?? && _item.value != "">
		<#local itemValue = _item.value>
	</#if>
	<#return itemValue>
</#function>

<#-- get page begin -->
<#function getPageBegin _context _page>
	<#local begin = _page.value * 6 + 1>
	<#if _context.configure??>
		<#local begin = _page.value * _context.configure.pageSize + 1>
	</#if>
	<#return begin>
</#function>

<#function getPageEnd _context _page>
	<#local end = _page.value * 6 + 6>
	<#if _context.configure??>
		<#local end = _context.configure.pageSize * (_page.value + 1)>
	</#if>
	<#if end gt _context.dataSize>
		<#local end = _context.dataSize>
	</#if>
	<#return end>
</#function>
