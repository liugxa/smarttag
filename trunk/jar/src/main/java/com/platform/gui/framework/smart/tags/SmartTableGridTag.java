...
public class SmartTableGridTag extends TagSupport {
	...
	public int doEndTag() throws JspException {
		try{
			...
			IDataRecordList<?> dataRecords = parent.getDataRecords();
			IDataColumn dataColumn = parent.getDataColumn();
			IDataColumnDecorate dataColumnDecorate = parent.getDataColumnDecorate();
			
			// if can not get the cells from the JSP, get it from the bean
			if(cells.size() == 0) cells = SmartTableGridUtil.getSmartCellList(context, dataColumn, dataColumnDecorate);
			
			// get the row data from the context
			rows = SmartTableGridUtil.getSmartRowList(context, dataRecords);
